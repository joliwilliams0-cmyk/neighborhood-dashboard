...
st.markdown("Precision market intelligence. Integrating BLS wage data from **cleaned_cities_uniform.csv** with regional infrastructure metrics.")

# 2. DATA ENGINE
@st.cache_data
def load_and_parse_bls_data():
    try:
        # Utilizing the exact file name requested
        df = pd.read_csv("cleaned_cities_uniform.csv")
        df.columns = df.columns.str.strip().str.upper()
        # Clean currency and handle top-tier ceiling income markers
        ...
        st.error(f"Critical Data Error: Ensure 'cleaned_cities_uniform.csv' is in your repository. Error: {e}")
        return pd.DataFrame()
